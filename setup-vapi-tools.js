...
async function createTool(functionDef) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to create tool ${functionDef.name}:`, response.status, errorText);
      return null;
    }

    const tool = await response.json();
    console.log(`✅ Created tool: ${functionDef.name} (ID: ${tool.id})`);
    return tool;
  } catch (error) {
    console.error(`Error creating tool ${functionDef.name}:`, error.message);
    return null;
  }
}
...
async function setupAllTools() {
  ...
  for (const functionDef of functions) {
    if (existingNames.includes(functionDef.name)) {
      console.log(`⏭️ Tool ${functionDef.name} already exists, skipping...`);
      continue;
    }
