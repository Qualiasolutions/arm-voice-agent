...
import { fallbackDb } from './fallback-client.js';

const supabaseUrl = process.env.SUPABASE_URL;
const supabaseAnonKey = process.env.SUPABASE_ANON_KEY;
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY; // Fixed: was SUPABASE_SERVICE_KEY

const USE_FALLBACK = !supabaseUrl || !supabaseAnonKey;

if (USE_FALLBACK) {
  console.warn('⚠️ Supabase environment variables not configured, using fallback database:', {
    SUPABASE_URL: supabaseUrl ? 'Present' : 'Missing',
    SUPABASE_ANON_KEY: supabaseAnonKey ? 'Present' : 'Missing',
    SUPABASE_SERVICE_ROLE_KEY: supabaseServiceKey ? 'Present' : 'Missing'
  });
  console.warn('⚠️ System will use sample data for demonstrations. Configure Supabase for production use.');
}

// Client for public operations (with RLS)
export const supabase = USE_FALLBACK ? null : createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
...
export const supabaseAdmin = USE_FALLBACK ? null : createClient(supabaseUrl, supabaseServiceKey || supabaseAnonKey, {
  auth: {
...
export const db = {
  ...
  async searchProducts(query, limit = 10) {
    if (USE_FALLBACK) {
      return fallbackDb.searchProducts(query, limit);
    }
  ...
  async searchProductsByFTS(query, limit = 10) {
    if (USE_FALLBACK) {
      return fallbackDb.searchProductsByFTS(query, limit);
    }
  ...
  async getProductBySkuOrName(identifier) {
    if (USE_FALLBACK) {
      return fallbackDb.getProductBySkuOrName(identifier);
    }
  ...
  async createConversation(vapiCallId, phoneNumber, metadata = {}) {
    if (USE_FALLBACK) {
      return fallbackDb.createConversation(vapiCallId, phoneNumber, metadata);
    }
  ...
  async updateConversation(vapiCallId, updates) {
    if (USE_FALLBACK) {
      return fallbackDb.updateConversation(vapiCallId, updates);
    }
  ...
  async checkAvailability(appointmentTime, duration = 30) {
    if (USE_FALLBACK) {
      return fallbackDb.checkAvailability(appointmentTime, duration);
    }
  ...
  async createAppointment(appointmentData) {
    if (USE_FALLBACK) {
      return fallbackDb.createAppointment(appointmentData);
    }
  ...
  async getAvailableSlots(date, serviceType, duration = 30) {
    if (USE_FALLBACK) {
      return fallbackDb.getAvailableSlots(date, serviceType, duration);
    }
  ...
  async trackEvent(eventType, properties = {}, conversationId = null) {
    if (USE_FALLBACK) {
      return fallbackDb.trackEvent(eventType, properties, conversationId);
    }
  ...
  async getCustomerByPhone(phoneNumber) {
    if (USE_FALLBACK) {
      return fallbackDb.getCustomerByPhone(phoneNumber);
    }
  ...
  async getCustomerOrderHistory(phoneNumber, limit = 10) {
    if (USE_FALLBACK) {
      return fallbackDb.getCustomerOrderHistory(phoneNumber, limit);
    }
  ...
  async searchCustomersByName(name) {
    if (USE_FALLBACK) {
      return fallbackDb.searchCustomersByName(name);
    }
