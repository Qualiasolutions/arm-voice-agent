...
export class FunctionRegistry {
  ...
  static async init() {
    ...
    const { default: voiceOptimizationFunctions } = await import('./voice-optimization.js');
    const { default: customerMemoryFunctions } = await import('./customer-memory.js');
    const { default: contextualSearchFunctions } = await import('./contextual-search.js');
    
    // Advanced monitoring and search functions
    const { default: voiceQualityFunctions } = await import('./voice-quality-monitor.js');
    const { default: semanticSearchFunctions } = await import('./semantic-product-search.js');
    ...
    // Register MCP-enhanced functions
    Object.entries(voiceOptimizationFunctions).forEach(([name, handler]) => {
      this.register(name, handler);
    });

    Object.entries(customerMemoryFunctions).forEach(([name, handler]) => {
      this.register(name, handler);
    });

    Object.entries(contextualSearchFunctions).forEach(([name, handler]) => {
      this.register(name, handler);
    });

    // Register advanced monitoring functions
    Object.entries(voiceQualityFunctions).forEach(([name, handler]) => {
      this.register(name, handler);
    });

    Object.entries(semanticSearchFunctions).forEach(([name, handler]) => {
      this.register(name, handler);
    });

    this.initialized = true;
    console.log(`Function Registry initialized with ${this.functions.size} functions (including MCP-enhanced + monitoring)`);
  }
