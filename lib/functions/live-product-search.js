// Live Product Search - Real-time access to armenius.com.cy via Firecrawl API
...
// Search armenius.com.cy live using Firecrawl API
async function searchArmeniusLive(query, category, maxResults) {
  try {
    console.log(`🔍 Live search: "${query}" (category: ${category || 'all'})`);
    
    // Use direct Firecrawl API integration
    const firecrawlResult = await firecrawlClient.searchArmeniusProducts(query, maxResults);
    
    if (firecrawlResult.success && firecrawlResult.products.length > 0) {
      console.log(`✅ Firecrawl API found ${firecrawlResult.products.length} products`);
      return firecrawlResult.products;
    }
    
    console.log('⚠️ Firecrawl API returned no results, trying direct HTTP fallback...');
    
    // Fallback to direct HTTP search if Firecrawl fails
    ...
    if (directResults && directResults.length > 0) {
      console.log(`✅ Direct HTTP fallback found ${directResults.length} results`);
      return directResults;
    }
    
    throw new Error('All live search methods failed');
...
// Scrape individual product details using Firecrawl API
async function scrapeProductDetails(url) {
  try {
    console.log(`🕷️ Scraping product details: ${url}`);
    
    const scrapeResult = await firecrawlClient.scrapeProductPage(url);
    
    if (scrapeResult.success && scrapeResult.data) {
      // Extract structured product data
      const productData = scrapeResult.data.extract || scrapeResult.data;
      
      return {
        name: productData.name || 'Unknown Product',
        price: productData.price || null,
        inStock: productData.availability !== 'out of stock',
        specifications: productData.specifications || {},
        description: productData.description || '',
        url: url,
        source: 'firecrawl_detail'
      };
    }
