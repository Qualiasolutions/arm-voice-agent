...
// Helper function for natural checking phrases
function getRandomCheckingPhrase() {
  const phrases = [
    "Sure, give me a moment to check...",
    "Let me see what we have available...", 
    "Of course! Let me check our current stock...",
    "Give me a minute to look that up for you...",
    "Sure thing! Let me check what we have in stock...",
    "Let me quickly check our inventory for you..."
  ];
...
// Helper function to search uploaded CSV file via Vapi context
async function searchUploadedProducts(searchTerm, language, context) {
  console.log('📁 Searching uploaded product CSV file...');
  
  try {
    // When files are uploaded to Vapi, they're accessible through the context
    // We need to instruct the AI to search the uploaded product file
    const searchInstruction = `Search the uploaded product CSV file for products matching "${searchTerm}". 
    Focus on laptops, computers, and electronic products. 
    Return the product name, price, availability (interpret numbers as stock quantities), and key specifications.
    If multiple products match, return the top 3 most relevant ones.
    Use natural language in your response and always include: "You can visit us at our store at 171 Makarios Avenue, Nicosia or order through our website armenius.com.cy"`;
    
    // This will leverage Vapi's file access capabilities
    return {
      searchFileInstruction: searchInstruction,
      searchTerm: searchTerm,
      useUploadedFile: true,
      message: language === 'el'
        ? `Φυσικά! Άσε με να ελέγξω τι έχουμε διαθέσιμο για "${searchTerm}"...`
        : getRandomCheckingPhrase()
    };
  } catch (error) {
    console.error('❌ CSV file search failed:', error);
    return null;
  }
}

// Helper function to try live search directly when other methods fail  
async function tryLiveSearchDirectly(searchTerm, language, context) {
  console.log('🌐 Attempting direct live search due to other failures...');
...
const inventoryFunctions = {
  checkInventory: {
    ...
    async execute(params, context) {
      ...
      try {
        // FIRST PRIORITY: Search uploaded CSV file
        console.log('🏆 PRIORITY 1: Searching uploaded product CSV file...');
        const csvResult = await searchUploadedProducts(searchTerm, language, context);
        
        if (csvResult?.useUploadedFile) {
          // Return instruction for Vapi to search the uploaded file
          return {
            available: true,
            message: language === 'el'
              ? `Φυσικά! Άσε με να ελέγξω τι έχουμε διαθέσιμο για "${searchTerm}"...`
              : getRandomCheckingPhrase(),
            searchFileInstruction: csvResult.searchFileInstruction,
            source: 'uploaded_csv_file',
            priority: 1
          };
        }
        
        // Test database connectivity as backup
        ...
        // If database fails, try uploaded file first, then live search
        const csvResult = await searchUploadedProducts(searchTerm, language, context);
        if (csvResult?.useUploadedFile) {
          return csvResult;
        }
