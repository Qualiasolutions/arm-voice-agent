...
async function searchUploadedProducts(searchTerm, language, context) {
  ...
  try {
    ...
    Return the product name, price, availability status, and key specifications.
    ...
    return {
      ...
      message: language === 'el'
        ? `Ελέγχω τον κατάλογο προϊόντων μας για "${searchTerm}"...`
        : `Checking our product catalog for "${searchTerm}"...`
    };
  } catch (error) {
      ...
      try {
        ...
        if (csvResult?.useUploadedFile) {
          ...
          return {
            available: true,
            message: language === 'el'
              ? `Ελέγχω τον πλήρη κατάλογο των 1000 προϊόντων μας για "${searchTerm}". Μια στιγμή παρακαλώ...`
              : `I'm checking our complete catalog of 1000 products for "${searchTerm}". One moment please...`,
