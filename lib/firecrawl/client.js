...
// Temporary: Disable Firecrawl import for deployment
...
class ArmeniusFirecrawlClient {
  ...
  async init() {
    ...
    try {
      // Temporary: Skip FirecrawlApp initialization during deployment
      // this.client = new FirecrawlApp({ apiKey: this.apiKey });
      this.initialized = false; // Keep as false to use fallback
      console.log('⚠️ Firecrawl client temporarily disabled for deployment');
      return false;
    } catch (error) {
